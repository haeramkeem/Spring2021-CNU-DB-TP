...
function libraryController(request, response) {
    ...
    libraryService.refreshDB().then(function (dbRefreshed) {
        if (dbRefreshed) {
            if (request.method === "POST") {
                ...
                if (reqUrl === "/signin") {
                    ...
                    //예약요청
                    postHandler(request).then(function (handleResult) {
                        var isbn = handleResult.isbn, id = handleResult.id;
                        if (typeof isbn === "string" && typeof id === "string") {
                            libraryService.doReserveBook(isbn, id).then(function (reserveResult) {
            ...
            //대비하지 못한 요청
            responseHandler(response, new classDomain_1.Responsable(409, "409 : Conflict"));
        }
    });
}
