...
function libraryController(request: http.IncomingMessage, response: http.ServerResponse) {
    ...
    libraryService.refreshDB().then(dbRefreshed => {
        if(dbRefreshed) {
            if(request.method === "POST") {
                ...
                if(reqUrl === "/signin") {
                    ...
                    //예약요청
                    postHandler(request).then(handleResult => {
                        let {isbn, id} = handleResult;
                        if(typeof isbn === "string" && typeof id === "string") {
                            libraryService.doReserveBook(isbn, id).then(reserveResult => {
            ...
            //대비하지 못한 요청
            responseHandler(response, new Responsable(409, "409 : Conflict"));
        }
    });
}
